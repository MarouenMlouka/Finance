...
def main():
    ...
    # Choix du produit
    choix_produit = st.selectbox("Choisissez un produit", list(produits.values()), key="choix_produit")
    ...
    ventes_initiales = st.text_input(f"Entrez les ventes totales par mois de {choix_produit} avant l'implémentation du projet (T0) :", key="ventes_initiales")
    try:
    ...
    for mois in range(1, periode_mesure + 1):
        ventes_mois = st.number_input(f"Entrez les ventes totales pour le mois {mois} (en boîtes) :", min_value=0, step=1, key=f"ventes_mois_{mois}")
    ...
    if st.button("Calculer les boîtes supplémentaires vendues"):
        if ventes_initiales is not None and all(ventes_mensuelles):
            ...
            st.warning("Veuillez remplir toutes les informations nécessaires avant de calculer.")
